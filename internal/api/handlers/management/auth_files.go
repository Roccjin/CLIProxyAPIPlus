...
func (h *Handler) GetAuthFileModels(c *gin.Context) {
	...
	authID := name
	if h.authManager != nil {
		if auth, ok := h.lookupAuthFile(name, strings.TrimSpace(c.Query("auth_index"))); ok && auth != nil {
			authID = auth.ID
		}
	}

	c.JSON(200, gin.H{"models": authFileModelEntries(registry.GetGlobalRegistry().GetModelsForClient(authID))})
}
