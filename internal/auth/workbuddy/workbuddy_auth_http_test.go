...
func newTestAuth(serverURL string) *WorkBuddyAuth {
...
func TestFetchAuthState_Success(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.URL.Path; got != workBuddyStatePath {
			t.Errorf("expected path %s, got %s", workBuddyStatePath, got)
		}
		if got := r.URL.Query().Get("platform"); got != PlatformCN {
			t.Errorf("expected platform=%s, got %s", PlatformCN, got)
		}
		if got := r.Header.Get("User-Agent"); got != UserAgentAuthCN {
			t.Errorf("expected User-Agent %s, got %s", UserAgentAuthCN, got)
		}
		if got := r.Header.Get("X-Domain"); got != "www.workbuddy.cn" {
			t.Errorf("expected X-Domain www.workbuddy.cn, got %s", got)
		}
...
func TestFetchAuthState_GlobalDomainHeader(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if got := r.URL.Query().Get("platform"); got != PlatformIntl {
			t.Errorf("expected platform=%s, got %s", PlatformIntl, got)
		}
		if got := r.Header.Get("User-Agent"); got != UserAgentAuthIntl {
			t.Errorf("expected User-Agent %s, got %s", UserAgentAuthIntl, got)
		}
		...
		_ = json.NewEncoder(w).Encode(map[string]any{
			...
			"data": map[string]any{
				"state":   "global-state",
				"authUrl": "https://www.workbuddy.ai/login?state=global-state",
			},
		})
	}))
	defer srv.Close()

	auth := NewWorkBuddyAuthForSite(nil, SiteGlobal())
...
func TestRefreshToken_Success(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.URL.Path; got != workBuddyRefreshPath {
			t.Errorf("expected path %s, got %s", workBuddyRefreshPath, got)
		}
	...
	if storage.Type != "workbuddy" {
		t.Errorf("expected type 'workbuddy', got '%s'", storage.Type)
	}
}
